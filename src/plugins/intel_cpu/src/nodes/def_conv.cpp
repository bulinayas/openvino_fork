...
#endif
bool DeformableConvolution::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
...
inline T nd_iterator_init(T start, U& x, const W& X, Args&&... tuple) {
    start = nd_iterator_init(start, static_cast<Args&&>(tuple)...);
    x = start % X;
    return start / X;
}
...
inline bool nd_iterator_step(U& x, const W& X, Args&&... tuple) {
    if (nd_iterator_step(static_cast<Args&&>(tuple)...)) {
        if (++x - X == 0) {
            x = 0;
            return true;
        }
...
inline void balance211(T n, U team, U tid, T& n_start, T& n_end) {
    ...
    if (team <= 1 || n == 0) {
        ...
        assert((T)team);
        T n1 = static_cast<typename std::remove_reference<T>::type>((n + (T)team - 1) / (T)team);
        T n2 = n1 - 1;
        T T1 = n - n2 * (T)team;
        n_my = (T)tid < T1 ? n1 : n2;
...
void deformable_convolution_cpu(const T* in,
                                ...
                                const std::vector<std::ptrdiff_t>& pads_begin,
                                ...
                                const bool bilinear_interpolation_pad)
{
    ...
    const int IH = in_shape[2];
    const int IW = in_shape[3];
    const int OC = out_shape[1] / groups;
    const int IC = in_shape[1] / groups;
    const int HW = OH * OW;
    ...
    const int KSH = strides[0];
    const int KSW = strides[1];

    const int KDH = dilation[0] - 1;
    const int KDW = dilation[1] - 1;

    const int padT = pads_begin[0];
    const int padL = pads_begin[1];

    const bool with_bi_pad = bilinear_interpolation_pad;

    const int G = groups;
    const int DG = deformable_groups;
...
void DeformableConvolution::DefConvRefExecutor::exec(const float* src,
    ...
    const std::vector<int>& in_shape{jcp.mb, jcp.ic * groups, jcp.ih, jcp.iw};
    const std::vector<int>& filter_shape{jcp.oc * groups, jcp.ic * groups, jcp.kh, jcp.kw};
    const std::vector<int>& out_shape{jcp.mb, jcp.oc * groups, jcp.oh, jcp.ow};
    const std::vector<int>& offset_shape{
        jcp.mb,
        deformable_groups * filter_shape[2] * filter_shape[3] * 2,
        out_shape[2],
        out_shape[3],
    };
